    ...
    <div className="min-h-screen bg-[#f8fafc] pb-24 text-[#1e293b]">
        ...
        <div className="grid grid-cols-2 gap-4">
          {isSenior ? (
            ...
            <>
              <Link
                href="/scan"
                className="bg-white p-6 rounded-3xl shadow-xl shadow-black/5 flex flex-col items-center gap-3 transition-all active:scale-95 border border-slate-100"
              >
                <div className="h-14 w-14 rounded-2xl bg-[#2563eb]/10 flex items-center justify-center text-[#2563eb]">
                  <QrCode className="h-7 w-7" />
                </div>
              ...
              <Link
                ...
                className="bg-white p-6 rounded-3xl shadow-xl shadow-black/5 flex flex-col items-center gap-3 transition-all active:scale-95 border border-slate-100"
              >
                <div className="h-14 w-14 rounded-2xl bg-orange-400/10 flex items-center justify-center text-orange-500">
                  <Trophy className="h-7 w-7" />
                </div>
                <span className="font-bold text-[#1e293b]">Ranking</span>
              </Link>
            ...
            {scans && scans.length > 0 ? (
              scans.map((scan: any) => (
                <div key={scan.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
                  ...
                  <div className="bg-[#2563eb]/10 text-[#2563eb] px-3 py-1 rounded-full text-xs font-bold border border-[#2563eb]/10">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-slate-100 px-8 py-4 flex justify-around items-center">
        <Link href="/" className="flex flex-col items-center gap-1 text-[#2563eb]">
          <div className="bg-[#2563eb]/10 p-2 rounded-xl">
            <UserIcon className="h-6 w-6" />
          </div>
        </Link>
        {isFreshman && (
          <>
            <Link href="/scan" className="flex flex-col items-center gap-1 text-[#64748b]">
              <QrCode className="h-6 w-6" />
            </Link>
            <Link href="/leaderboard" className="flex flex-col items-center gap-1 text-[#64748b]">
              <Trophy className="h-6 w-6" />
            </Link>
          </>
        )}
        {isSenior && (
          <Link href="/senior/qr" className="flex flex-col items-center gap-1 text-[#64748b]">
            <QrCode className="h-6 w-6" />
          </Link>
        )}
      </div>
